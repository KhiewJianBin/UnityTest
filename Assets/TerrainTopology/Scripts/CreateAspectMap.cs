...
public class CreateAspectMap : CreateTopologyMap
{
    public enum ASPECT_TYPE { ASPECT, NORTHERNESS, EASTERNESS };
    public ASPECT_TYPE m_aspectType = ASPECT_TYPE.ASPECT;

    protected override Texture2D CreateMap(float[] heights, int width, int height)
    {
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                float aspect = 180.0f * (1.0f + Mathf.Sign(dx)) - Mathf.Sign(dx) * a;
